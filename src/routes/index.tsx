  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "AI Arena— AI Agent Battles" },
      {
        ...
        content: "Collect, train, and battle unique AI Agents in a Web3 arena.",
      },
    ],
  }),
  component: Landing,
  ...
  return (
    <header className="sticky top-0 z-50 backdrop-blur-xl bg-background/60 border-b border-border">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between gap-3 sm:gap-6">
        ...
        <nav className="hidden lg:flex items-center gap-5 text-sm xl:gap-7">
          {links.map((link) => (
            <a
              key={link.label}
              href={link.href}
              className="relative transition text-muted-foreground hover:text-foreground"
            >
              {link.label}
            </a>
          ))}
        ...
        <div className="flex shrink-0 items-center gap-2 sm:gap-3">
          <div className="flex items-center gap-1.5 rounded-full border border-border bg-card/50 px-2 py-1 sm:gap-2 sm:px-3 sm:py-1.5">
            <a
              href="/achievements"
              title="View Achievements"
              className="w-5 h-5 sm:w-5.5 sm:h-5.5 rounded-full overflow-hidden border border-white/10 hover:border-primary transition shrink-0 active:scale-95 flex items-center justify-center"
            >
              <img
                src={dashboardAvatar}
                alt="Achievements avatar"
                className="w-full h-full object-cover"
              />
            </a>
            <a href="/leaderboard" className="flex items-center gap-1.5" title="View $ARENA rankings">
              <span className="font-tech text-[10px] sm:text-xs">$ARENA</span>
              <span className="font-tech text-[10px] sm:text-xs text-accent">1.00</span>
            </a>
          </div>
          <button
            type="button"
          ...
          <nav className="max-w-7xl mx-auto px-4 sm:px-6 py-4 grid gap-2">
            {links.map((link) => (
              <a
                ...
                className="rounded-md px-3 py-3 text-sm transition hover:bg-card/70 text-muted-foreground hover:text-foreground"
              >
                {link.label}
              </a>
            ))}
    ...
    <section className="max-w-7xl mx-auto px-4 sm:px-6 py-12 sm:py-16">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-6 text-center sm:text-left">
        ...
        <div className="flex items-center justify-between sm:justify-end gap-4">
          <a href="/my-agents" className="text-sm text-accent hover:underline">
            View All
          </a>
  ...
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 py-12 sm:py-16">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3 mb-6 text-center sm:text-left">
        ...
        <a href="/battles" className="text-sm text-accent hover:underline">
          View All Battles
        </a>
      </div>
        ...
        {battles.map((b, i) => (
          <div key={i} className="card-glass rounded-xl p-4 sm:p-5">
            ...
            <div className="flex items-center justify-between mt-4 pt-3 border-t border-border/50">
              <a href="/battles" className="text-xs text-accent hover:underline font-tech">
                Watch Now
              </a>
          ...
          <div key={c.title}>
            ...
            <ul className="space-y-2">
              {c.items.map((i) => (
                <li key={i}>
                  <a
                    href={i === "Leaderboard" ? "/leaderboard" : i === "Moments" ? "/moments" : "#"}
